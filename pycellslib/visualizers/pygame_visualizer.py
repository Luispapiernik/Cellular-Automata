...
class System(object):
    ...
    """
    def __init__(self, automaton, colors, null_cell=0, clear=True, export=True,
                 add=True):
        self.automaton = automaton
        # para que la configuracion inicial se pueda ver en pantalla
        self.automaton.topology.flip()
        self.automaton.topology.set_values_from_configuration(self.automaton.topology.get_states(), None)
        self.automaton.topology.flip()
    ...
    def add(self, pos, _, added=None):
        ...
        if 0 <= x < self.width and 0 <= y < self.height:
            ...
            # para que el cambio se pueda ver en pantalla toca hacer unn flip
            self.automaton.topology.flip()
            self.automaton.topology.update_cell((y, x), self.states[(index + 1) % len(self.states)], None)
            self.automaton.topology.flip()
            self.automaton.topology.update_cell((y, x), self.states[(index + 1) % len(self.states)], None)
    ...
    def draw(self, screen):
        ...
        for i in range(self.system.height):
            for j in range(self.system.width):
                p.draw.rect(screen, self.system.get_color(i, j),
    ...
    def eventManager(self, screen):
        ...
        for event in set_events():
            ...
            if event.type == p.MOUSEBUTTONUP:
                self.system.mouse_button_up(
                    self.getPositionInMatrix(event.pos), event)
            if event.type == p.MOUSEMOTION:
                self.system.mouse_motion(
                    self.getPositionInMatrix(event.pos), event)
